...
#include <stdio.h>
volatile spinlock spinlock_absysig={.flag=0};
unsigned long buffersize_absysig = 2;
volatile token_absysig arr_absysig[2];
circularFIFO_absysig channel_absysig;
...
void init_circularFIFO_absysig(circularFIFO_absysig* channel ,token_absysig* buffer,size_t size){
	...
	dst:data
	*/
inline int read_circularFIFO_non_blocking_absysig(circularFIFO_absysig* channel, token_absysig* data){
	...
	*/
inline int write_circularFIFO_non_blocking_absysig(circularFIFO_absysig* channel, token_absysig value){
...
inline	int read_circularFIFO_blocking_absysig(circularFIFO_absysig* channel, token_absysig* data,spinlock* lock){
		spinlock_get(lock);
		if(channel->front==channel->rear){
		    	//empty 
...
inline int write_circularFIFO_blocking_absysig(circularFIFO_absysig* channel, token_absysig value,spinlock* lock){
	spinlock_get(lock);
