...
inline static void read_channel_Gy_gy(circularFIFO_gysig* src_channel_ptr, const size_t num, token_gysig  dst[]){
	//#if defined SINGLE
...
inline static void write_channel_Gy_resy(token_absysig src[],const size_t num,circularFIFO_absysig* dst_channel_ptr){
	for(size_t i=0 ; i < num ;++i){
...
inline void actor_Gy(circularFIFO_gysig* channel_gy_ptr, const size_t gy_rate
