...
inline static void read_channel_Gx_gx(circularFIFO_gxsig* src_channel_ptr, const size_t num, token_gxsig  dst[]){
	//#if defined SINGLE
...
inline static void write_channel_Gx_resx(token_absxsig src[],const size_t num,circularFIFO_absxsig* dst_channel_ptr){
	for(size_t i=0 ; i < num ;++i){
...
inline void actor_Gx(circularFIFO_gxsig* channel_gx_ptr, const size_t gx_rate
